...
void QR::mult_Q_by_R(size_t i_start)
{
    ...
    // +- same time for sub_block_size2 = 48, 96 (96 a bit longer)
...
void QR::mult_W_by_V(size_t size)
{
    //W is (n)*(i) matrix starting from [0][0] (F)

    size_t W_row = n - i_start;
    size_t W_col = size;

    //V is (i)*(n) matrix starting from [0][0] (S)

    size_t V_col = n - i_start;

    const int block_size_row = 72, block_size_col = 144;

    const int sub_block_size = 6, sub_block_size2 = 72, sub_block_size3 = 8; //sub_sub_block_size3 == 2 * sizeof(__m512); (byte)
    // +- same time for sub_block_size2 = 48, 96

    int t = W_row - (W_row % sub_block_size);// i
    int l = V_col - (V_col % sub_block_size3);// j
    int s = W_col - (W_col % sub_block_size2);// k

#pragma omp parallel for
    for (int i1 = 0; i1 < W_row; i1 += block_size_row)
        for (int k1 = 0; k1 < W_col; k1 += block_size_col)
            for (int j1 = 0; j1 < V_col; j1 += block_size_row)
                    ...
                    for (int k3 = k1; k3 < k1 + block_size_col && k3 < s; k3 += sub_block_size2)
                        for (int j3 = j1; j3 < j1 + block_size_row && j3 < l; j3 += sub_block_size3)
                        {
                           ...
                           */
                            ...
                            for (int i4 = 0; i4 < sub_block_size; i4++)
                                for (int j4 = 0; j4 < 2; j4++)
                                    c[(i4 << 1) + j4] = _mm256_loadu_pd(&q_tmp((i4 + i3), j3 + j4 * (sub_block_size3 >> 1)));

                            for (int k5 = 0; k5 < sub_block_size2; k5++)
                            {
                                b1 = _mm256_loadu_pd(&V((k3 + k5),j3));
                                b2 = _mm256_loadu_pd(&V((k3 + k5),j3 + (sub_block_size3 >> 1)));
#pragma unroll(sub_block_size)
                                for (int i5 = 0; i5 < sub_block_size; i5++)
                                {
                                    int i = i5 << 1;

                                    a = _mm256_set1_pd(W((i3 + i5),(k3 + k5)));
                            ...
                            for (int i6 = 0; i6 < sub_block_size; i6++)
                                for (int j6 = 0; j6 < 2; j6++)
                                    _mm256_storeu_pd(&q_tmp((i6 + i3), j3 + j6 * (sub_block_size3 >> 1)), c[(i6 << 1) + j6]);
...
#pragma omp parallel for
            for (int i = 0; i < W_row; i++)
                for (int k = s; k < W_col; k++)
#pragma omp simd
                    for (int j = 0; j < V_col; j++)
                        q_tmp(i, j) += W(i, k) * V(k, j);

            //int t = F.row - (F.row % block_size_row);// i
#pragma omp parallel for
            for (int i = t; i < W_row; i++)
                for (int k = 0; k < s; k++)
#pragma omp simd
                    for (int j = 0; j < V_col; j++)
                        q_tmp(i, j) += W(i, k) * V(k, j);
        }
        else if ((W_row != t) && (W_col == s))
        {
            //int t = F.row - (F.row % block_size_row);// i
#pragma omp parallel for
            for (int i = t; i < W_row; i++)
